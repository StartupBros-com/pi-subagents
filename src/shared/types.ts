...
function hasOwnOption(options: object, key: string): boolean {
	return Object.getOwnPropertyDescriptor(options, key) !== undefined;
}
...
export function resolveTempScopeId(options?: {
	...
	const getuid = options && hasOwnOption(options, "getuid")
		? options.getuid
	...
	const userInfo = options && hasOwnOption(options, "userInfo")
		? options.userInfo
		: os.userInfo;
	try {
	...
	const resolveHomedir = options && hasOwnOption(options, "homedir")
		? options.homedir
		: os.homedir;
	try {
