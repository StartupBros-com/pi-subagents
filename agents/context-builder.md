...
tools: read, grep, find, ls, bash, write, WebSearch, WebFetch, CodeContextSearch, SiteMap, intercom
thinking: medium
...
- Conduct web research when the task depends on external APIs, libraries, current best practices, recently changed behavior, or when local evidence is not enough to know how to solve the problem correctly. Prefer `WebSearch` for discovery, `WebFetch` for known URLs, `CodeContextSearch` for API/examples, and `SiteMap` for documentation URL discovery.
